...
class BusinessImpactAnalyzer:
    ...
    def analyze(self, input_text):
        try:
            # More specific and focused prompt
            prompt = (
    ...
    def format_analysis(self, text):
        ...
        for sentence in sentences:
            ...
            # If no clear indicators, use context
            elif 'cost' in sentence.lower() or 'expense' in sentence.lower():
                effect_text.append(sentence)
            else:
                cause_text.append(sentence)

        # Ensure we have content in both sections
        if not cause_text:
    ...
    def clean_text(self, text):
        # Remove generic business theory and filler content
        cleanup_patterns = [
            r'in general,?',
            r'typically,?',
